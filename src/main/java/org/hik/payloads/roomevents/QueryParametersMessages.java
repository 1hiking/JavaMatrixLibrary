...
///
/// @param from  the token to start returning events from. This token can be obtained from a prev\_batch or next\_batch token returned by the /sync endpoint, or from an end token returned by a previous request to this endpoint.
///
/// This endpoint can also accept a value returned as a start token by a previous request to this endpoint, though servers are not required to support this. Clients should not rely on the behavior.
///
...
/// @param to    The token to stop returning events at. This token can be obtained from a prev\_batch or next\_batch token returned by the /sync endpoint, or from an end token returned by a previous request to this endpoint.
